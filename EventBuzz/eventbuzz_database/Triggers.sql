...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is 
-- error during insert on Users table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        INSERT INTO EventBuzzAuditTables.ErrorLog (error_source, error_message)
        VALUES ('after_user_insert', 'Error occurred');
    END;
    INSERT INTO EventBuzzAuditTables.UserLog (general_id, action_type, details)
    VALUES (NEW.user_id, 'registration', CONCAT('user_id: ', NEW.user_id, ', user_name: ', NEW.username));
END$$
DELIMITER ;

-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is 
-- error during update on Users table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is 
-- error during delete on Users table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on EventCategories table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on EventCategories table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on EventCategories table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on Venues table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on Venues table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on Venues table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on Events table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on Events table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on Events table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on Orders table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on Orders table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on Orders table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on Tickets table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on Tickets table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on Tickets table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on Reviews table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on Reviews table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on Reviews table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on Sponsors table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on Sponsors table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on Sponsors table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on Organisers table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        INSERT INTO EventBuzzAuditTables.ErrorLog (error_source)
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on Organisers table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on Organisers table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on Notifications table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on Notifications table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on Notifications table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on NotificationsSendToUsers table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on NotificationsSendToUsers table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on NotificationsSendToUsers table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on UsersRegisterForEvents table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        INSERT INTO EventBuzzAuditTables.ErrorLog (error_message, error_context)
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on UsersRegisterForEvents table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on UsersRegisterForEvents table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on EventsFundedBySponsors table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        INSERT INTO EventBuzzAuditTables.ErrorLog (error_message, error_context, event_name)
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on EventsFundedBySponsors table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on EventsFundedBySponsors table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        INSERT INTO EventBuzzAuditTables.ErrorLog (error_context, event_name)
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during insert on EventsOrganisedByOrganisers table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during update on EventsOrganisedByOrganisers table, then insert into `EventBuzzAuditTables`.ErrorLog table.
...
-- Trigger to insert logs into `EventBuzzAuditTables`.UserLog table. If there is
-- error during delete on EventsOrganisedByOrganisers table, then insert into `EventBuzzAuditTables`.ErrorLog table.
